...
class UserManager(BaseUserManager):
    ...
    def _create_user(self, username, phone_number, email, password, is_staff, is_superuser, **extra_fields):
        """
        creates and saves a user with the given username, email and passsword
        """
        now = timezone.now()
        if not username:
    ...
    def create_user(self, username=None, phone_number=None, email=None, password=None, **extra_fields):
        ...
        email = email or ''  # Provide a default email if not provided
...
class User(AbstractUser, PermissionsMixin):
    username = models.CharField(_('user name'), max_length=32, unique=True,
                                help_text=_(
                                    'Required 30 charactes or fewer starting with letter'),
                                validators=[
                                    validators.RegexValidator(r'^[a-zA-Z][a-zA-Z0-9_\.]+$',
                                                              _('Enter a valud username starting with a-z.'
